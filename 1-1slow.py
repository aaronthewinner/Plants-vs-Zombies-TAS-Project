...
def slowFiftyRule(waves):
    ...
    for i in range(1,waves):
        ...
        if(i % 10 == 9):
            continue
        WriteMemory("int",(int)(ReadMemory("int",0x6a9ec0, 0x768, 0x5598)*0.5), 0x6a9ec0, 0x768, 0x5594)
            ...
            if not zombie_dead and zombie_age <= 50 and ReadMemory("int", zombies_offset + 0x24 + i * 0x15c) == zombie_Types[j]:
                if(zombie_Types[j] == 3):
                    ...
                    if(read_memory("int", 0x6A9EC0, 0x768, 0x557C) % 10 == 0):
                        ...
                        WriteMemory("float",819, zombies_offset + 0x2c + i * 0x15c) # worst spawn x-positions
                if(zombie_rows[0] != 0):
...
Prejudge(0,4)
WriteZombies([0,0],[3,3],[0.23,0.23])
