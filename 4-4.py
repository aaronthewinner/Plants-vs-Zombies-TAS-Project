...
def WriteZombies(zombie_Types,zombie_rows=[0]):
    ...
    while(j < len(zombie_Types)):
        ...
        for i in range(zombies_count_max):
            ...
            zombie_hypnod = ReadMemory("bool", zombies_offset + 0xb8 + i * 0x15c)
            if not zombie_dead and not zombie_hypnod and ReadMemory("int", zombies_offset + 0x8 + i * 0x15c) > 750 and ReadMemory("int", zombies_offset + 0x24 + i * 0x15c) == zombie_Types[j]:
                if(zombie_Types[j] == 3):
...
Prejudge(0,9)
WriteZombies([0,0,0],[6,6,6])
PlantWhenAvailable(2,0,(1,9))
PlantWhenAvailable(1,25,(6,2))
PlantWhenAvailable(5,50,(6,9))
PlantWhenAvailable(6,25,(5,9))

Prejudge(0,10)
WriteZombies([0,0,0,0,0,1,2,16,16],[6,6,6,6,6,6,6,4,4])
use_shovel(2,9)
PlantWhenAvailable(3,0,(3,9))
PlantWhenAvailable(2,0,(2,6))

Prejudge(0,11)
WriteZombies([2,16],[2,3])
PlantWhenAvailable(7,75,(2,9))

Prejudge(0,12)
WriteZombies([0,0,16],[1,2,5])
PlantWhenAvailable(2,0,(2,9))

Prejudge(0,13)
WriteZombies([0,0,0,16],[5,5,5,6])
PlantWhenAvailable(2,0,(5,7))

Prejudge(0,14)
WriteZombies([0,16,16],[3,4,2])
PlantWhenAvailable(2,0,(5,8))

Prejudge(0,15)
WriteZombies([0,16,16],[5,3,1])
PlantWhenAvailable(2,0,(5,6))

Prejudge(0,16)
WriteZombies([0,0,16,16],[6,6,5,5])
use_shovel(5,9)
use_shovel(2,9)
use_shovel(1,9)

Prejudge(0,17)
WriteZombies([0,0,16,16],[2,2,6,6])

Prejudge(0,18)
WriteZombies([0,0,16,16],[1,1,2,2])

Prejudge(0,19)
WriteZombies([0,16,16,16],[1,1,1,1])
use_shovel(3,9)
PlantWhenAvailable(5,50,(1,9))

Prejudge(1,20)
WriteZombies([0,0,0,0,0,0,0,1,2,2,14,16],[1,2,3,4,5,6,1,2,3,5,4,6])
